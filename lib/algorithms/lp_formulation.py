...
from .abstract_formulation import SiteAbstractFormulation, ServerAbstractFormulation, Objective, Solver
...
class SiteLP_Formulation(SiteAbstractFormulation):
    @classmethod
    ...
    @staticmethod
    def paths_full_fname(problem, num_paths, edge_disjoint, dist_metric, top): 
        if top:       
            return os.path.join(PATHS_DIR_site_top, '{}-{}-paths_edge-disjoint-{}_dist-metric-{}-dict.pkl'.format(problem.name, num_paths,
            edge_disjoint, dist_metric))
        else:
            return os.path.join(PATHS_DIR_site, '{}-{}-paths_edge-disjoint-{}_dist-metric-{}-dict.pkl'.format(problem.name, num_paths, 
            edge_disjoint, dist_metric))
    ...
    def get_paths(self, problem,top):
        paths_fname = SiteLP_Formulation.paths_full_fname(problem, self._num_paths, self.edge_disjoint, self.dist_metric,top)
    ...
    def pre_solve(self, top,problem=None):
        if problem is None:
        ...
        paths_dict = self.get_paths(problem,top)
        path_i = 0
    ...
    def _construct_lp(self, top, sat_flows=[]):
...
class ServerLP_Formulation(ServerAbstractFormulation):
    @classmethod
    def new_max_flow(cls, num_paths, edge_disjoint=True, dist_metric='inv-cap', out=None):
        print('Objective', Objective)
        return cls(objective=0,
        ...
        if self._solver_type == Solver.Gurobi:
            ...
            if self._objective == 0:
                ...
                def obj_rule(m):
                    ...
                    return result
                m.obj = Objective(rule=obj_rule, sense=maximize)
                #m.obj = Objective(expr=sum(m.path_vars[i] for i in m.path_vars), sense=maximize)
            elif self._objective == Objective.MAX_CONCURRENT_FLOW:
                self._print('MAX CONCURRENT FLOW objective')
                self.alpha = m.addVar(vtype=GRB.CONTINUOUS, lb=0.0, ub=1.0, name='a')
                m.update()
            ...
            if self.DEBUG:
                m.write('lp_debug.lp')
            return LpSolver_pyomo(m, None, self.DEBUG, self.VERBOSE, self.out)

    @staticmethod
    def paths_full_fname(problem, num_paths, edge_disjoint, dist_metric, server): 
        return os.path.join(PATHS_DIR_srv,'srv_paths-{}'.format(server),'{}-{}-paths_edge-disjoint-{}_dist-metric-{}-dict.pkl'.format(problem.name, num_paths, edge_disjoint, dist_metric))
    ...
    def get_paths(self, problem, server):
        paths_fname = ServerLP_Formulation.paths_full_fname(problem, self._num_paths, self.edge_disjoint, self.dist_metric,server)
        self._print('Loading paths from pickle file', paths_fname)
        try:
            with open(paths_fname, 'rb') as f:
                paths_dict = pickle.load(f)
    ...
    def pre_solve(self, server, problem=None):
        if problem is None:
            problem = self.problem
        ...
        self._path_to_commod = {}
        self._all_paths = []
        self.n_server = server

        paths_dict = self.get_paths(problem, server)
        path_i = 0
        for k, (s_k, t_k, d_k) in self.commodity_list:
            # print('dict:', paths_dict)
            paths = paths_dict[(s_k, t_k)]
    ...
    def _construct_lp(self, server, sat_flows=[]):
        edge_to_paths, num_paths = self.pre_solve(server)
        #print(edge_to_paths)
        self._print('Constructing Path LP')
        return self._construct_path_lp(self._problem.G, edge_to_paths,
                                       num_paths, sat_flows)
