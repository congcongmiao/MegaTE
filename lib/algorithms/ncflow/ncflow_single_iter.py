...
from ...lp_solver_gurobi import LpSolver_gurobi, Method
...
class NCFlowSingleIter(NCFlowAbstract):
    ...
    def _r1_lp(self, paths_dict, meta_commodity_list):
        ...
        #print('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nr1time:', end_r1_time-start_r1_time)
        #with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
        #    t.write('{} '.format(end_r1_time-start_r1_time))
    ...
    def _r2_lp(self, curr_meta_node, paths_dict, G_hat, all_v_hat_in,
               all_v_hat_out, intra_commods, min_max_util=False):
        debug_r2 = False
        ...
        #print('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nr2time:', end_r2_time-start_r2_time)
        #with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
        #    t.write('{} '.format(end_r2_time-start_r2_time))
    ...
    def _r3_lp(self, meta_commodities, constrain_r3_by_r1=True):
        ...
        #print('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nr3time:', end_r3_time-start_r3_time)
        #with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
        #    t.write('{} '.format(end_r3_time-start_r3_time))
    ...
    def solve(self,
        ...
        with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt', 'a') as t:
            t.write('\nr1 lp time:{},sync time:{}\n'.format(self._runtime_dict['r1'],self._synctime_dict['r1']))
        ...
        with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
            t.write('\nr2 lp time:{},sync time:{}\n'.format(sum([t for _, t in self._runtime_dict['r2'].items()]),sum([t for _, t in self._synctime_dict['r2'].items()])))
        ...
        with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
            t.write('\nreconciliation lp time:{},sync time:{}\n'.format(sum([t for _, t in self._runtime_dict['reconciliation'].items()]),sum([t for _, t in self._synctime_dict['reconciliation'].items()])))
        # Kirchoff's
        self._print('\nKirchoff\'s')
        ...
        with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
            t.write('\nkirchoffs lp time:{},sync time:{}\n'.format(sum([t for _, t in self._runtime_dict['kirchoffs'].items()]),sum([t for _, t in self._synctime_dict['kirchoffs'].items()])))
        ...
        self._synctime_dict['r3'] += time.time() - start_time
        with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
            t.write('\nr3 lp time:{},sync time:{}\n'.format(self._runtime_dict['r3'],self._synctime_dict['r3']))
        ...
        #record r3 solution by paths
        #for commodity, flow in self.r3_sol_paths.items():
        #    for path_id, d in flow:
        #        path = self._r3_paths[path_id]
        #        if path not in self.final_sol_paths
        #            self.final_sol_paths[tuple(path)] += 
        ...
        self._print('-->> Runtime= ', self.runtime_est(16))
        self.sol_dict_as_paths
