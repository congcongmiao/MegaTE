from ...config import TOPOLOGIES_DIR, TL_DIR
...
class NCFlowEdgePerIter(AbstractFormulation):
    ...
    def __init__(self, *, objective, num_paths, edge_disjoint, dist_metric, DEBUG, VERBOSE, out=None, **args):
        ...
        #record accumulated r3 solution across all iteration
        #self.final_sol_paths = {}
    ...
    def compute_r2_paths_for_meta_node(self, meta_node_id, iter):
        print('computing r2 paths for meta node...')
        before_r2_path_time = time.time()
        ...
        with open(full_fname, 'wb') as w:
            pickle.dump(paths_dict, w)
        after_r2_path_time = time.time()
        print('computing r2 paths time:',after_r2_path_time-before_r2_path_time)
        with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
            t.write('{} '.format(after_r2_path_time-before_r2_path_time))
    ...
    def solve(self, problem, partitioner):
        ...
        for iter in range(self.max_num_iters):
            ...
            self.iter_time[-1] += time.time()-start_time
            with open(os.path.join(TL_DIR, 'benchmarks', 'ncflow-logs') + '/runtime_log.txt','a') as t:
                t.write('\nIteration{},computing residual problem time:{}\n'.format(iter, self.iter_time[-1]))
        ...
        if not hasattr(self, '_sol_dict'):
            ...
            for nc in self._ncflows:
                ...
                for (_, (s_k, t_k, _)), flow_list in sol_dict.items():
                    #print('self_soldict_flow_list:', flow_list)
