...
class Problem(object):
    def __init__(self,
        ...
        if traffic_matrix is not None:
            if isinstance(traffic_matrix, np.ndarray):
                ...
                self.traffic_matrix = GenericTrafficMatrix(self, traffic_matrix, seed, scale_factor=scale_factor)
    ...
    ############################
    @classmethod
    def from_file(cls, topology_fname, traffic_matrix_fname, top, old_way=True):
        if topology_fname.endswith('-edgelist.txt'):
            ...
            G = Problem._read_graph_graphml(topology_fname, top)
            old_G = None
    ...
    def _read_graph_graphml(fname, top):
        ...
        #for scc_ids in nx.weakly_connected_components(file_G):
