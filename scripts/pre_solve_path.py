...
from lib.algorithms.path_formulation import PathFormulation, PATHS_DIR, PATHS_DIR_site
...
if __name__ == '__main__':
    ...
    #else:
    problem = get_problem(sys.argv[1], model='uniform', random=False)
    ...
    if not os.path.exists(PATHS_DIR_site):
        os.makedirs(PATHS_DIR_site)

    paths_fname = PathFormulation.paths_full_fname(problem, num_paths,
                                                   edge_disjoint, dist_metric,False)
    print(paths_fname)
    if LOAD_FROM_DISK:
